...
void set_profcfg(int val, int op)
{
	...
	if (str == NULL)
  	...
  	snprintf(str, 7, "%d", val);
   	...
   	{
     		fputs(RED "[Sperf]" RESET " Invalid option in set_profcfg", stderr);
      		exit(1);
   	}
   	free(str);
}
...
char * get_path(char * argmnt, int location)
{
	...
	if (location == ETC_PATH)	
		strcat(path, "/../etc/sperf_exec.conf");
...
void exec_conf(int * l_num_exec, int * l_num_max_threads, char * exec_path)
{
	...
	{
		fprintf(stderr, RED "[Sperf]" RESET " Failed to open sperf_exec.conf: %s\n", strerror(errno));
		exit(1);
	}

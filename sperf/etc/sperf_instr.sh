...
cleaning()
{
	...
	if [[ $NUM_PRAGMAS -gt 0 ]]
	then
		echo -e "${BLUE}[sperf_instr.sh]${NC} Cleaning $FPARSE"
		sed -i -e 's/sperf_start();//g' $FPARSE
		sed -i -e 's/sperf_stop();//g' $FPARSE
		sed -i '/^[^/].*sperfops.h"$/d' $FPARSE
	else
		echo -e "${YELLOW}[sperf_instr.sh]${NC} Nothing to cleaning in $FPARSE"
	fi
}
...
set_start()
{
	case "$1" in
		...
		then
			sed -i "$LINE i sperf_start();" $FPARSE
		fi
		;;
			...
			do
				...
				then
					...
					then
						...
						sed -i "$LINE a sperf_stop();" $FPARSE
						break
					fi
...
echo -e "${BLUE}[sperf_instr.sh]${NC} Retriving the application's source code path"
# Path to sperf_instr.sh
EXEC_PATH=$(dirname $0)
cd $EXEC_PATH
CONF_PATH=$PWD
...
do
	...
	do
		...
		echo -e "${BLUE}[sperf_instr.sh]${NC} $FPARSE"
		...
		case "$1" in 
			...
			then
				LOCAL_MARK_COUNT=1
				PRAGMA_COUNT=1
				sed -i '1 i #include "'$INCLUDE_PATH'/sperfops.h"' $FPARSE
				...
				echo -e "${BLUE}[sperf_instr.sh]${NC} $(( $LOCAL_MARK_COUNT-1 )) marks on $FPARSE file"
				
			else
				echo -e "${YELLOW}[sperf_instr.sh]${NC} Nothing to parse on $FPARSE"
			fi
			;;
...
case "$1" in
	...
	echo -e "${BLUE}[sperf_instr.sh]${NC} Total of $(( $MARK_COUNT-1 )) marks done"
	;;
esac
